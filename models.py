...
class Laptop(db.Model):
    ...
    model = db.Column(
        db.String(500),
        nullable=False)  # Increased length for longer model names
    price = db.Column(db.Float, nullable=False)
    product_url = db.Column(db.String(1000),
                            nullable=True)  # URL to the product page

    # Basic specifications
    cpu = db.Column(db.String(200),
                    nullable=False)  # Increased length for CPU descriptions
    gpu = db.Column(db.String(200),
                    nullable=True)  # Increased length for GPU descriptions
    ...
    display_resolution = db.Column(db.String(50),
                                   nullable=False)  # e.g., 1920x1080
    ...
    gaming_fps = db.Column(db.Float,
                           nullable=True)  # Average FPS in common games

    # User ratings and categorization
    user_rating = db.Column(db.Float, nullable=True)  # 1-5 scale
    build_quality = db.Column(db.String(100),
                              nullable=True)  # Poor, Average, Good, Excellent
    value_category = db.Column(db.String(50),
                               nullable=True)  # Budget, Mid-Range, High-End
...
class UserPreference(db.Model):
    ...
    # Other preferences
    preferred_display_size = db.Column(db.String(20),
                                       nullable=True)  # Small, Medium, Large
    preferred_weight = db.Column(db.String(20),
                                 nullable=True)  # Light, Medium, Heavy
    battery_life_importance = db.Column(db.Integer, nullable=True)  # 1-5 scale
    build_quality_importance = db.Column(db.Integer,
