...
def index():
    ...
    with app.app_context():
        ...
        laptops = Laptop.query.order_by(
            Laptop.user_rating.desc()).limit(6).all()

        # Get categories for filtering
        categories = {
            'gaming':
            Laptop.query.filter_by(suitable_for_gaming=True).count(),
            'business':
            Laptop.query.filter_by(suitable_for_business=True).count(),
            'student':
            Laptop.query.filter_by(suitable_for_students=True).count(),
            'content_creation':
            Laptop.query.filter_by(suitable_for_content_creation=True).count()
        }
...
def search():
    ...
    with app.app_context():
        ...
        laptops = Laptop.query.filter(
            or_(Laptop.brand.ilike(f'%{query}%'),
                Laptop.model.ilike(f'%{query}%'))).all()
...
def submit_questionnaire():
    ...
    with app.app_context():
        ...
        user_pref = UserPreference(
            ...
            brand_pref=brand_pref,  # Store brand preference
...
def results(pref_id):
    ...
    with app.app_context():
        ...
        user_prefs_dict = {
            'use_case':
            'gaming' if user_pref.use_case_gaming else
            'business' if user_pref.use_case_business else
            'student' if user_pref.use_case_student else 'content_creation'
            if user_pref.use_case_content_creation else 'general',
            'budget_min':
            user_pref.budget_min,
            'budget_max':
            user_pref.budget_max,
        ...
        if len(recommended_laptops) < 3:
            ...
            if value_ids:
                value_laptops = Laptop.query.filter(
                    ...
                    ~Laptop.id.in_(recommended_ids) if recommended_ids else
                    True  # Exclude already recommended laptops
                ).all()
        ...
        if recommended_laptops:
            similar_ids = laptop_recommender.find_similar_laptops(
                recommended_laptops[0].id, limit=2)
            alternatives = Laptop.query.filter(
                Laptop.id.in_(similar_ids)).all() if similar_ids else []
...
def laptop_detail(laptop_id):
    ...
    with app.app_context():
        ...
        # Get similar laptops
        similar_ids = laptop_recommender.find_similar_laptops(laptop_id,
                                                              limit=3)
        ...
        if 'user_session_id' in session:
            is_favorite = Favorite.query.filter_by(
                session_id=session['user_session_id'],
                laptop_id=laptop_id).first() is not None
    ...
    return render_template(
        ...
        similar_laptops=[l.to_dict()
                         for l in similar_laptops] if similar_laptops else [],
...
def comparison():
    ...
    with app.app_context():
        # Get the laptops
        laptops = Laptop.query.filter(
            Laptop.id.in_(laptop_ids)).all() if laptop_ids else []
...
def add_to_favorites(laptop_id):
    ...
    try:
        with app.app_context():
            ...
            existing = Favorite.query.filter_by(session_id=session_id,
                                                laptop_id=laptop_id).first()
...
def remove_from_favorites(laptop_id):
    ...
    with app.app_context():
        ...
        favorite = Favorite.query.filter_by(session_id=session_id,
                                            laptop_id=laptop_id).first()
